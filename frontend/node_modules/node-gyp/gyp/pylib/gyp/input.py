...
def ProcessListFiltersInDict(name, the_dict):
  ...
  for key, value in the_dict.iteritems():
    ...
    if type(the_dict[list_key]) is not list:
      ...
      raise ValueError(
          f'{name} key {list_key} must be list, not {value.__class__.__name__} when applying '
          + {
              '!': 'exclusion',
              '/': 'regex'
          }[operation])
  ...
  for list_key in lists:
    ...
    if regex_key in the_dict:
      for regex_item in the_dict[regex_key]:
        ...
        if action == 'exclude':
          ...
          raise ValueError(f'Unrecognized action {action} in {name} key {regex_key}')
    ...
    if excluded_key in the_dict:
      raise GypError(
          f'{name} key {excluded_key} must not be present prior  to applying exclusion/regex filters for {list_key}'
      )
...
def ValidateTargetType(target, target_dict):
  ...
  if target_type not in VALID_TARGET_TYPES:
    raise GypError(
        f"Target {target} has an invalid target type '{target_type}'.  Must be one of {'/'.join(VALID_TARGET_TYPES)}."
    )
  if (target_dict.get('standalone_static_library', 0)
      and target_type != 'static_library'):
    raise GypError(
        f'Target {target} has type {target_type} but standalone_static_library flag is only valid for static_library type.'
    )
